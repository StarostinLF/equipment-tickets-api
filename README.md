...
1. `requestId` — назначает `req.id` (или берёт из заголовка `X-Request-Id` клиента), чтобы и логгер, и обработчик ошибок могли на него ссылаться.
2. `requestLogger` (pino-http) — логирует метод, путь, код ответа и длительность каждого запроса вместе с его id.
...
5. `notFoundHandler` — несуществующие маршруты отдают 404 в общем формате ошибки.
...
Настраивается отдельным этапом (CORS, rate limiting, helmet) — раздел будет дополнен.
